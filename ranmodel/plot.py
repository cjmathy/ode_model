...
def plot_all_species_separate_plots(y,t,species,args,output):
	'''
	Description:
		This method plots the concentration curves obtained from odeint. The user can choose between various plotting styles from the command line.
	Input:
	...
	Returns: None
	'''
	if args.plot is "all": species_plotted = species
	else: species_plotted = args.plot.split(",")
	for sp in species_plotted:
		if sp in species:
			...
			plt.xticks(np.arange(0, int(args.t)+1, int(args.t)/2))
				# plt.yticks(np.arange(0, 1.1*float(max(y[:,i])),0.1*float(max(y[:,i]))))
			plt.grid(True)
			# plt.tight_layout()
			fig.savefig(output + 'fig_%s.png' %sp)
	return
	...
	for i in range(y.shape[1]):
		...
		for sp in species:
			if species[sp].index is i:
				...
				# plt.yticks(np.arange(0, 1.1*float(max(y[:,i])),0.1*float(max(y[:,i]))))
				plt.grid(True)
	fig.suptitle("All Species, total time = %s seconds" %args.t,fontsize=16)
	plt.tight_layout()
...
def plot_species_percent(y,t,species,args,output):
	...
	nuc_tot = species['GTP'].conc0+species['GDP'].conc0+species['RanGTP'].conc0+species['RanGDP'].conc0
	fig = plt.figure()
...
def plot_ran_ratios(y,t,species,args,output,figure):
	...
	plotted_species = ['nucRanGTP','nucRanGDP','nucRanGTP/nucRanGDP','nucRanGTP/cytoRanGTP','cytoRanGTP','cytoRanGDP','cytoRanGTP/cytoRanGDP','nucRanGDP/cytoRanGDP']
	...
	for each in plotted_species:
		...
		if each == 'nucRanGTP/nucRanGDP':
			...
			ax.plot(t, y[:,species['nucRanGTP'].index]/y[:,species['nucRanGDP'].index])
			plt.title(each)
			plt.ylabel('Ratio')
			plt.xlabel('Time (seconds)')
			plt.xticks(np.arange(0, int(args.t)+1, int(args.t)/2))
			...
			ax.plot(t, y[:,species['nucRanGTP'].index]/y[:,species['cytoRanGTP'].index])
			plt.title(each)
			plt.ylabel('Ratio')
			plt.xlabel('Time (seconds)')
			plt.xticks(np.arange(0, int(args.t)+1, int(args.t)/2))
			...
			ax.plot(t, y[:,species['cytoRanGTP'].index]/y[:,species['cytoRanGDP'].index])
			plt.title(each)
			plt.ylabel('Ratio')
			plt.xlabel('Time (seconds)')
			plt.xticks(np.arange(0, int(args.t)+1, int(args.t)/2))
			...
			ax.plot(t, y[:,species['nucRanGDP'].index]/y[:,species['cytoRanGDP'].index])
			plt.title(each)
			plt.ylabel('Ratio')
			plt.xlabel('Time (seconds)')
			plt.xticks(np.arange(0, int(args.t)+1, int(args.t)/2))
			...
			# ax = fig.add_subplot(2, 3, plotted_species.index(each)+1)
