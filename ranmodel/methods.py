...
from ranmodel.utils import *

def parseArguments():
	'''
	Description:
		This method parses additional arguments passed when the script is called.
	Input:
		None
	Returns:
	...
	parser.add_argument("-species", default='species.csv', type=str, help = "name of species CSV file. Default is to look for 'species.csv' in the package directory")
	parser.add_argument("-ode", default='ode.csv', type=str, help = "name of ode CSV file. Default is to look for 'ode.csv' in the package directory")
	parser.add_argument("-parameters", default='parameters.csv', type=str, help = "name of parameters CSV file. Default is to look for 'parameters.csv' in the package directory")
	parser.add_argument("-out", default=None, type=str, help = "name of desired output directory. Default is to create a folder 'output' if one does not already exist in the package directory")
	parser.add_argument("-n", default=1000, help="integer for the number of iterations of numerical integration, default is 1000")
	parser.add_argument("-t", default=3600, help="integer for the total time (seconds) modeled, default is 3600")
	parser.add_argument("-plot", default='all', help = "string of the names of each species to be plotted, separated by commas") #TO ADD
	return parser.parse_args()

def importSpecies(speciesfile,odefile):
	'''
	Description:
		This method imports two csv files containing molecular species information, according to the formats described in the README file. The first file should contain initial concentration values for each species, while the second file should contain the ODE string for each species. The information for each species is stored as an object of the Species Class, and then stored in a dictionary. The keys of the dictionary are strings corresponding to the name of each species, and the value of each key is the corresponding Species object.
	Input:
		speciesfile - a string containing the csv filename (including path, if necessary) for the file containing Species and initial concentrations.
		odefile - a string containing the csv filename (including path, if necessary) for the file containing Species and ODE equations.
	Returns:
...
def runmodel(species,parameters,sysargs):
	...
	concentrations = odeint(odeSystem,c0,t,args=(species,parameters))
...
def initializeConcentrations(species):
	...
	for s in species: c0[species[s].index] = species[s].conc0
	return c0
...
def odeSystem(y,t,species,parameters):
	...
	'''
	for s in species: exec("%s = %.100f" % (s,y[int(species[s].index)]))
	for p in parameters: exec("%s = %.100f" % (p,parameters[p].value))
	dydt = [None]*len(species)
	for s in species: dydt[int(species[s].index)] = eval(species[s].ode)
	return dydt
